...
export type HealthApiSource =
  | 'openfda' | 'rxnav' | 'umls' | 'dailymed' | 'pubmed'
  | 'drugbank' | 'goodrx' | 'webmd' | 'evidencemd' | 'infermedica';
...
function kindTitle(kind: HealthApiKind, query: Record<string, any>): string {
  ...
  const labels: Record<HealthApiKind, string> = {
    'openfda-label': 'Label',
    'openfda-ae': 'Adverse reactions',
    'openfda-recall': 'Recall',
    'rxnav-ddi': 'Interactions',
    'rxnav-info': 'Drug info',
